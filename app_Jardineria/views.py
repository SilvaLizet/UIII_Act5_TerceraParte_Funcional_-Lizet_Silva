...
def detalle_cliente(request, cliente_id):
    ...
    # Para mostrar los servicios asociados al cliente
...
def detalle_empleado(request, empleado_id):
    ...
    # Para mostrar los servicios asignados al empleado
    servicios_asignados = empleado.servicios_asignados.all() # Accede a través del related_name en ContratoServicio
...
def agregar_servicio(request):
    if request.method == 'POST':
        ...
        activo = True if 'activo' in request.POST else False # Checkbox
...
def detalle_servicio(request, servicio_id):
    ...
    # Para mostrar los contratos donde este servicio está incluido
...
# --- VISTAS CRUD CONTRATO (PARA MOSTRAR RELACIONES) ---
...
# Vista para agregar un contrato, útil para crear las relaciones
def agregar_contrato(request):
    ...
    if request.method == 'POST':
        ...
        empleados_ids = request.POST.getlist('empleados') # getlist para ManyToMany
        ...
        contrato = ContratoServicio.objects.create(
            ...
            costo_total=costo_total if costo_total else None # Manejar campo opcional
        )
        contrato.empleados.set(empleados_ids) # Asigna los empleados al contrato
...
def actualizar_contrato(request, contrato_id):
    ...
    if request.method == 'POST':
        ...
        # Actualizar la relación ManyToMany para empleados
        empleados_ids = request.POST.getlist('empleados')
        contrato.empleados.set(empleados_ids) # set() actualiza la relación

        return redirect('ver_contratos')
    
    # Empleados seleccionados para la plantilla (para pre-seleccionar)
